...
```solidity
function __ERC3525Upgradeable_init() public
```
...
```solidity
function _parseData(bytes data) internal pure virtual returns (struct HyperCertMinter.Claim claim, uint64[])
```
...
```solidity
function _getBackgroundIndex(string primaryScopeOfImpact) internal view returns (uint256 index)
```
...
```solidity
function _generateFooter(struct HyperCertSVG.SVGParams params, struct HyperCertSVG.SVGColors colors) internal pure virtual returns (string)
```
